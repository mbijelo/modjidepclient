...
class Card50 extends StatefulWidget {
  ...
  final Color? color;
  final String textNodispo;
  final String  sendDemandText;
  final Function xxx;
  ...
  Card50({
    ...
    required this.textNodispo,
    required this.sendDemandText,
    required this.xxx
  });
...
class _Card50State extends State<Card50>{
  ...
  Widget build(BuildContext context) {
   ...
   // Widget image = widget.item.gallery.isNotEmpty ? showImage(widget.item.gallery[0].serverPath, fit: BoxFit.cover) : Container();
    Widget image = widget.providerData.logoServerPath.isNotEmpty ? showImage(widget.providerData.logoServerPath, fit: BoxFit.cover) : Container();
            ...
            Expanded(child: Column(
              ...
              children: [
                ...
                Container(
                    margin: widget.direction == TextDirection.ltr ? EdgeInsets.only(right: 30) : EdgeInsets.only(left: 30),
                   // child: Text(getTextByLocale(widget.item.name, widget.locale), style: aTheme.style14W800, textAlign: TextAlign.start,)),
                    child: Text(getTextByLocale(widget.providerData.name, widget.locale), style: aTheme.style14W800, textAlign: TextAlign.start,)),
                ...
                if (widget.categoryEnable )
                        Wrap(
                          ...
                          children: getCategories(
                              widget.item.category, widget.locale,
                              widget.category)
                              .map((e) {
                            return GestureDetector(
                                onTap: widget.providerData.visible && widget.item.visible ? () {
                                  widget.xxx();
                                } : () {},
                              child: Container(
                              ...
                              decoration: BoxDecoration(
                                //color: Colors.green,
                                color: widget.providerData.visible && widget.item.visible ? Colors.green : Colors.red,
                                borderRadius: BorderRadius.circular(
                                    aTheme.radius),
                              ),
                             // child: Text(e, style: aTheme.style12W600White,),
                              child: Text(
                                widget.providerData.visible && widget.item.visible ? widget.sendDemandText : widget.textNodispo,
